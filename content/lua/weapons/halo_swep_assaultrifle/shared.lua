...
SWEP.Gun = ("halo_swep_assaultrifle") -- must be the name of your swep but NO CAPITALS!
SWEP.Category				= "Halo" --Category where you will find your weapons
SWEP.Author				= "Sixx"
SWEP.Contact				= "eve-gaming.com"
SWEP.Instructions				= "LMB: Fire"
SWEP.PrintName				= "MA37 Assault Rifle"		-- Weapon name (Shown on HUD)	
...
function SWEP:DetectionHUD(trace)
	...
	surface.SetTexture(surface.GetTextureID("VGUI/crosshairs/assaultrifle"))
...
function SWEP:Reload()
	...
	-- When the current clip < full clip and the rest of your ammo > 0, then
		...
		self.Weapon:SetNetworkedBool("Reloading", true)
						...
						timer.Simple( 0.5, function() if SERVER then if not IsValid(self) then return end
		...
		self.Weapon:SetNetworkedBool("Reloading2", true)
				...
				timer.Simple( 2.7, function() if SERVER then if not IsValid(self) then return end
...
function SWEP:Think()
	...
	if self.Weapon:GetNetworkedBool("Reloading2") or self.Owner:KeyDown(IN_ATTACK) then
		self.VElements["ar"].bodygroup = {[2] = 1}
		else
		self.VElements["ar"].bodygroup = {[2] = 0}
	end
